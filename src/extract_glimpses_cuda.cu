...
at::Tensor crop2d_gpu(
  ...
  if (X.dim() == 2) {
    channels = 1;
    off = 1;
    //output = X.type().zeros({R.size(0), pooled_height, pooled_width});
    output = at::zeros({R.size(0), pooled_height, pooled_width}, X.type());
  } else if (first) {
    channels = X.size(0);
//     output = X.type().zeros(
//         {R.size(0), channels, pooled_height, pooled_width});
    output = at::zeros({R.size(0), channels, pooled_height, pooled_width}, X.type());
  } else {
    channels = X.size(2);
//     output = X.type().zeros(
//         {R.size(0), pooled_height, pooled_width, channels});
    output = at::zeros({R.size(0), pooled_height, pooled_width, channels}, X.type());
  }
...
//         {R.size(0), pooled_length, pooled_height, pooled_width});
    output = at::zeros({R.size(0), pooled_length, pooled_height, pooled_width}, X.type());
  } else if (first) {
    channels = X.size(0);
//     output = X.type().zeros(
//         {R.size(0), channels, pooled_length, pooled_height, pooled_width});
    output = at::zeros({R.size(0), channels, pooled_length, pooled_height, pooled_width}, X.type());
  } else {
    channels = X.size(3);
//     output = X.type().zeros(
//         {R.size(0), pooled_length, pooled_height, pooled_width, channels});
    output = at::zeros({R.size(0), pooled_length, pooled_height, pooled_width, channels}, X.type());
  }
...
//      X.type(), "crop2d", [&](){
//      using scalar_t = at::cuda::type<scalar_t>;
//      //scalar_t s = 1;
//      });
//}
//
