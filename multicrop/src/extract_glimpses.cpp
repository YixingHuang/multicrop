...
at::Tensor crop2d_cpu(
  ...
  if (X.dim() == 2) {
    ...
    off = 1;
    output = at::zeros(
        {R.size(0), pooled_height, pooled_width}, X.type());
  } else if (first) {
    channels = X.size(0);
    output = at::zeros(
        {R.size(0), channels, pooled_height, pooled_width}, X.type());
  } else {
    channels = X.size(2);
    output = at::zeros(
        {R.size(0), pooled_height, pooled_width, channels}, X.type());
  }
...
at::Tensor crop3d_cpu(
  ...
  if (X.dim() == 3) {
    ...
    off = 1;
    output = at::zeros(
        {R.size(0), pooled_length, pooled_height, pooled_width}, X.type());
  } else if (first) {
    channels = X.size(0);
    output = at::zeros(
        {R.size(0), channels, pooled_length, pooled_height, pooled_width}, X.type());
  } else {
    channels = X.size(3);
    output = at::zeros(
        {R.size(0), pooled_length, pooled_height, pooled_width, channels}, X.type());
  }
